...
local WZ   = "Wichization-NikaidouHiro"
local SHERRY_WZ = "Witchization-TachibanaSherry"
local REB  = "Rebuttal"
local RW   = "Rewind"
...
local function repose_current_count()
    ...
    if allies ~= nil then
        for _, u in ipairs(allies) do
            if st(u, WZ) > 0 or st(u, SHERRY_WZ) > 0 then n = n + 1 end
        end
    end
...
function hiro_curse_rs()
    ...
    if w > 0 then
        ...
        drain_sp(clamp(math.floor(w / 2), 0, 15))                  -- 回合开始降(层/2)理智
        ...
        -- 物理三抗封顶到×0.5(耐性)已并入 [Wichization-NikaidouHiro] buff 本体
...
function hiro_immortal()
    if st(SELF, RWW) > 0 or st(SELF, RW) > 0 then
        setimmortal(1)
    else
        setimmortal(0)
...
function hiro_power()
    ...
    elseif id == 9750121 then
        -- 解脱：获得10层魔女化；魔女化超出50每层+2%伤害(最多100)；sum/6基础(最多4)；wf/10最终(最多10)；不降低理智
        setdata(SELF, D_DELIV_BGM_HELD, 0)
        setdata(SELF, D_DELIV_BGM_NEW, 0)
        buff(SELF, WZ, 10, 0, 0)
        local w = st(SELF, WZ)
        if w > 50 then dmgmult(clamp((w - 50) * 2, 0, 100)) end
...
function hiro_def_witch_endround()
    ...
    getstat(pick, "HiroSetSpCorrosion")                    -- 直接将理智设为 -45，不走理智伤害流程
...
function hiro_ritual_after()
    ...
    each_unit(function(u)
        ...
        destroybuff(u, SHERRY_WZ, 0, 99, 1)
